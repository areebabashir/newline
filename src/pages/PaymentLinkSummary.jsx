...
import { Search, Calendar, Download, Plus, X } from 'lucide-react';
import {
  createColumnHelper,
  flexRender,
  getCoreRowModel,
...
const PaymentLinkSummary = () => {
  ...
  const handleCreateLink = (e) => {
    e.preventDefault();
    console.log("New Payment Link:", newLink);
          ...
          <div className="flex flex-col sm:flex-row justify-between items-start sm:items-center gap-4">
            <div>
              ...
              <p className="text-sm text-gray-600 dark:text-gray-400">
                Manage all payment links and their details
              </p>
            </div>
            <button
            ...
            <div className="bg-white dark:bg-gray-800 rounded-2xl shadow-lg w-full max-w-lg p-6 relative">
              ...
              <form onSubmit={handleCreateLink} className="space-y-4">
                <input
                  ...
                  value={newLink.accountName}
                  onChange={(e) =>
                    setNewLink({ ...newLink, accountName: e.target.value })
                  }
                ...
                <input
                  ...
                  value={newLink.jobNo}
                  onChange={(e) =>
                    setNewLink({ ...newLink, jobNo: e.target.value })
                  }
                  className="w-full px-4 py-3 border rounded-xl bg-white dark:bg-gray-700 text-gray-900 dark:text-white"
                  required
                />
                <input
                  type="text"
                  placeholder="Remarks"
                  value={newLink.remarks}
                  onChange={(e) =>
                    setNewLink({ ...newLink, remarks: e.target.value })
                  }
                  className="w-full px-4 py-3 border rounded-xl bg-white dark:bg-gray-700 text-gray-900 dark:text-white"
                />
                <input
                  ...
                  value={newLink.date}
                  onChange={(e) =>
                    setNewLink({ ...newLink, date: e.target.value })
                  }
                  className="w-full px-4 py-3 border rounded-xl bg-white dark:bg-gray-700 text-gray-900 dark:text-white"
                  required
                />
                <div className="grid grid-cols-2 gap-4">
                  <input
                    ...
                    value={newLink.net}
                    onChange={(e) =>
                      setNewLink({ ...newLink, net: e.target.value })
                    }
                    className="px-4 py-3 border rounded-xl bg-white dark:bg-gray-700 text-gray-900 dark:text-white"
                  />
                  <input
                    type="number"
                    placeholder="VAT"
                    value={newLink.vat}
                    onChange={(e) =>
                      setNewLink({ ...newLink, vat: e.target.value })
                    }
                    className="px-4 py-3 border rounded-xl bg-white dark:bg-gray-700 text-gray-900 dark:text-white"
                  />
                </div>
                <button
                  type="submit"
                  className="w-full bg-blue-500 hover:bg-blue-600 text-white py-3 rounded-xl font-medium shadow-md transition-all duration-200"
                >
                  Save Link
                </button>
              </form>
          ...
          <div className="flex flex-col sm:flex-row gap-4 items-start sm:items-center justify-between">
            <div className="flex flex-col sm:flex-row gap-4 items-start sm:items-center flex-1 min-w-0">
              ...
              <div className="flex flex-col sm:flex-row gap-2">
                ...
                />
                <span className="text-gray-600 dark:text-gray-400 text-sm self-center">to</span>
                <input
                  type="date"
                  className="px-3 py-2 border border-gray-200 dark:border-gray-600 rounded-lg focus:outline-none focus:ring-2 focus:ring-blue-500 focus:border-transparent bg-white dark:bg-gray-700 text-gray-900 dark:text-white text-sm"
                  value={endDate}
              ...
              <button
                className="px-3 py-2 bg-gray-100 dark:bg-gray-600 text-gray-700 dark:text-gray-300 rounded-lg hover:bg-gray-200 dark:hover:bg-gray-500 transition-colors duration-200 text-sm"
                onClick={resetFilters}
              >
                Reset
              </button>
            </div>
            <button className="flex items-center gap-2 px-3 py-2 bg-gray-100 dark:bg-gray-600 text-gray-700 dark:text-gray-300 rounded-lg hover:bg-gray-200 dark:hover:bg-gray-500 transition-colors duration-200 text-sm">
              <Download size={18} />
              Export CSV
            </button>
          </div>
                  ...
                  {table.getHeaderGroups().map((headerGroup) => (
                    <tr key={headerGroup.id}>
                      {headerGroup.headers.map((header) => (
                        <th
                          key={header.id}
                          className="text-left py-4 px-3 font-semibold text-gray-900 dark:text-white cursor-pointer select-none whitespace-nowrap text-xs"
                  ...
                  {table.getRowModel().rows.map((row) => (
                    ...
                    >
                      {row.getVisibleCells().map((cell) => (
                        <td key={cell.id} className="py-4 px-3 whitespace-nowrap text-xs">
