...
const AffiliateList = () => {
  ...
  const getInviteButton = (canReceive, affiliateId) => (
    <button
      ...
      className={`px-3 py-1 rounded-full text-sm font-medium transition-colors duration-150 ${
        canReceive
          ? 'bg-yellow-100 text-yellow-600 dark:bg-yellow-900/30 dark:text-yellow-400 hover:bg-yellow-200 dark:hover:bg-yellow-800/30 cursor-pointer'
  ...
  const getSendInviteButton = (canSend, affiliateId) => (
    <button
      ...
      className={`px-3 py-1 rounded-full text-sm font-medium transition-colors duration-150 ${
        canSend
          ? 'bg-amber-100 text-amber-600 dark:bg-amber-900/30 dark:text-amber-400 hover:bg-amber-200 dark:hover:bg-amber-800/30 cursor-pointer'
          : 'bg-gray-100 text-gray-400 dark:bg-gray-700 dark:text-gray-500 cursor-not-allowed'
      }`}
    >
  ...
  const columns = [
    ...
    columnHelper.accessor('companyName', {
      header: 'Company Name',
      cell: (info) => (
        <span className="text-yellow-600 dark:text-yellow-400 font-medium hover:text-yellow-700 dark:hover:text-yellow-300 cursor-pointer">
          {info.getValue()}
        </span>
      ),
    }),
    ...
    columnHelper.accessor('operationCenter', {
      ...
      cell: (info) => (
        <span className="px-3 py-1 rounded-full text-sm font-medium bg-yellow-100 text-yellow-600 dark:bg-yellow-900/30 dark:text-yellow-400">
          {info.getValue()}
        </span>
      ),
    }),
        ...
        <div className="bg-white dark:bg-gray-800 rounded-2xl shadow-sm p-6">
          <div className="flex justify-between items-center">
            ...
            <button
              ...
              className="bg-yellow-500 hover:bg-yellow-600 dark:bg-yellow-700 dark:hover:bg-yellow-800 text-white px-6 py-3 rounded-full font-medium flex items-center gap-2 shadow-lg transition-all duration-200 hover:scale-105"
            >
              <Plus size={20} />
              INVITE MANUALLY
            </button>
